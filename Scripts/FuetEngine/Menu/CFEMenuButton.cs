...
namespace FuetEngine
{
	...
	*/
	...
	public class CFEMenuButton
	{
		...
		private	CFEMenuPage		m_oOwner;
		private CFEMenuButton[]	m_oNeighs = new CFEMenuButton[4]();
		...
		//-----------------------------------------------------------------------------
		void Init(CFEHUDObject _oObj,CFEHUDManager _oHUDManager)
		{
			// m_oOriObj		= CFEHUDInstancer::poCreateInstance(_poObj);
			m_oObj			= _poObj;
			m_oHUDManager	= _poHUDManager;
			m_oRect			= CFEHUDRectGen::oGetRect(NULL,_poObj);

			ChangeState(EFEMenuButtonState.MBS_NONE);
		}
		//-----------------------------------------------------------------------------
		void Finish()
		{
		...
		// classes.
		//-----------------------------------------------------------------------------
		void ProcessEvent(EFEMenuButtonEvent _eEventType)
		{
			switch (_eEventType)
			{
				/// When page enters
				...
				{
					m_uiNextState = m_state.GetState();
