...
namespace FuetEngine
{
	public class CFEHUDLoader
    {
		...
		public static CFEHUD oLoad(CFEString _sFilename)
		{
            ...
            CFEHUD oHUD = Support.CreateObject<Node2D>("res://addons/FuetEngine/HUD/CFEHUD.cs") as CFEHUD;
		...
		protected static CFEHUDElement oLoadElement(CFEString _sPrefix, CFEConfigFile _oConfigFile)
		{
			...
			CFEHUDElement oElem = Support.CreateObject<Node2D>("res://addons/FuetEngine/HUD/CFEHUDElement.cs") as CFEHUDElement;
		...
		protected static CFEHUDObject oLoadObject(string _sPrefix, CFEConfigFile _oConfigFile)
		{
			...
			{
				CFEHUDIcon oIcon = new CFEHUDIcon(sElemName);
				...
				if (sSprite != "")
				{
					...
					/*
                    CFESpriteInstance oSpriteInstance   = new CFESpriteInstance(); //  oGameObject.AddComponent<CFESpriteInstance>() as CFESpriteInstance;
					oSpriteInstance.m_sSpriteFile       = sSpriteFile;
                    oSpriteInstance.m_uiSpriteAction    = poIcon.iGetIniAction();
                    oSpriteInstance.m_oColor            = oRenderColor(m_oModColor, poIcon);
                    oSpriteInstance.m_iCreationOrder    = m_iObjectIdx;
                    // oSpriteInstance.Start();
                    oSpriteInstance.Init(oGameObject);

                    // oSpriteInstance.Setup();
                    GameObject.DestroyImmediate(oSpriteInstance);
                    */
                }
			...
			else if (sElemType == "Group")
			{
				...
				CFEHUDGroup oGroup = Support.CreateObject<Node2D>("res://addons/FuetEngine/HUD/CFEHUDGroup.cs") as CFEHUDGroup;
				oGroup.Name = sElemName;
